...
from fmengine.core.nn.transforms import Transform
...
class Message:
    ...
    def __init__(
        ...
        self.role = role
        self.content = [{"type": "text", "content": content}] if isinstance(content, str) else content
        self.masked = masked
    ...
    def __init__(
        ...
        if column_map:
            if "chosen" not in column_map:
                raise ValueError(f"Expected a key of 'chosen' in column_map but found {column_map.keys()}.")
            if "rejected" not in column_map:
                raise ValueError(f"Expected a key of 'rejected' in column_map but found {column_map.keys()}.")
            self._column_map = column_map
        else:
    ...
    def __call__(self, sample: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        for message in sample[self._column_map["chosen"]]:
            if message["role"] == "system" and self.new_system_prompt is not None:
                continue
            message["masked"] = (message["role"] != "assistant") and (not self.train_on_input)
        ...
        if self.new_system_prompt is not None:
            chosen_messages = [
                Message(role="system", content=self.new_system_prompt, masked=True, eot=True)
            ] + chosen_messages
            rejected_messages = [
                Message(role="system", content=self.new_system_prompt, masked=True, eot=True)
            ] + rejected_messages
...
class ShareGPTToMessages(Transform):
    ...
    def __init__(
        ...
        if column_map:
            if "conversations" not in column_map:
                raise ValueError(f"Expected a key of 'conversations' in column_map but found {column_map.keys()}.")
            self._column_map = column_map
        else:
    ...
    def __call__(self, sample: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        messages = []
        if self.new_system_prompt is not None:
            messages.append(Message(role="system", content=self.new_system_prompt, masked=True, eot=True))
...
class JSONToMessages(Transform):
    ...
    def __init__(
        ...
        if column_map:
            if "messages" not in column_map:
                raise ValueError(f"Expected a key of 'messages' in column_map but found {column_map.keys()}.")
            self._column_map = column_map
        else:
    ...
    def __call__(self, sample: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        updated_messages = []
        if self.new_system_prompt is not None:
            updated_messages.append(Message(role="system", content=self.new_system_prompt, masked=True, eot=True))
